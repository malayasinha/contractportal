...
public class LoginDAOImpl implements LoginDAO {
	...
	public Map<String, Object> validate(Map<String, Object> mapData) {
		...
		try {
			...
			String userRoleSql = "SELECT users.user_id,role.role_name,role.role_id FROM role,users WHERE role.role_id=users.role_id AND users.enabled='1' AND role.enabled='Y' AND users.username ='"
					+ userName + "' ";
			// userList=this.sessionFactory.getCurrentSession().createQuery("from
			...
			if (userList != null && userList.size() > 0) {
				...
				// String userRollAppSql= "select * from user_role_application
				// where role_id="+roleId;
				String userRollAppSql = "select app.app_id,app.name,app.url,app.parent_id from role_application as ura , application as app where app.app_id=ura.app_id and app.status='Y' and ura.role_id="
						+ roleId;
				...
				if (userRollAppList.size() > 0) {
					for (Iterator iterator = userRollAppList.iterator(); iterator.hasNext();) {
						Object[] objects = (Object[]) iterator.next();
			...
			usersList = this.sessionFactory.getCurrentSession().createQuery("from Users ul where ul.id=" + userId)
					.list();
			usersLogList = this.sessionFactory.getCurrentSession()
					.createQuery("from UsersLog ul where ul.usersId=" + userId + " order by id desc").list();
