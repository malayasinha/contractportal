...
public class DepartmentDAOImpl implements DepartmentDAO {
	...
	public Map<String, Object> editDepartment(Integer id) {
		...
		editDepartmentList = this.sessionFactory.getCurrentSession()
				.createQuery("from Department as ue where ue.id=" + id).list();
		departmentList = this.sessionFactory.getCurrentSession().createQuery("from Department").list();
	...
	public Map<String, Object> saveDepartment(Map<String, Object> map) {
		...
		uniquedata = (List<Department>) this.sessionFactory.getCurrentSession().createCriteria(Department.class)
				.add(Restrictions.eq("departmentName", entity.getDepartmentName())).list();
	...
	public Map<String, Object> modifyDepartment(Map<String, Object> map) {
		...
		try {
			...
			if (department != null && department.getId() != null && department.getId() > 0) {
				Department editDepartment = (Department) session.load(Department.class, department.getId());
