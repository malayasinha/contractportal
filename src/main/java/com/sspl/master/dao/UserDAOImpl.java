...
public class UserDAOImpl implements UserDAO {
	...
	public Map<String, Object> viewUser() {
		...
		employeList = this.sessionFactory.getCurrentSession().createQuery("from EmployeeEntity where status='1'")
				.list();
	...
	public Map<String, Object> editUser(Integer id) {
		...
		editUserList = this.sessionFactory.getCurrentSession().createQuery("from UsersEntity as ue where ue.id=" + id)
				.list();
		userList = this.sessionFactory.getCurrentSession().createQuery("from UsersEntity").list();
		// roleList=this.sessionFactory.getCurrentSession().createQuery("from
		// UsersRole ur where ur.usersEntity="+id).list();
		roleList = this.sessionFactory.getCurrentSession().createQuery("from Role").list();
	...
	public Map<String, Object> saveUser(Map<String, Object> map) {
		...
		uniquedata = (List<UsersEntity>) this.sessionFactory.getCurrentSession().createCriteria(UsersEntity.class)
				.add(Restrictions.eq("username", entity.getUsername())).list();
	...
	public Map<String, Object> modifyUser(Map<String, Object> map) {
		...
		try {
			...
			if (usersEntity != null && usersEntity.getId() != null && usersEntity.getId() > 0) {
				UsersEntity editUsersEntity = (UsersEntity) session.load(UsersEntity.class, usersEntity.getId());
			...
			/*
			 * String deleteQuery =
			 * "delete UsersRole ur where ur.usersEntity= :userroleid"; int
			 * value=session.createQuery(deleteQuery).setInteger("userroleid",
